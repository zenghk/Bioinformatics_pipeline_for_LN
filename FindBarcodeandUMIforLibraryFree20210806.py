...
def main():
	...
	existsfile = glob.glob("%s/*R1*.fastq"%outdir)
	if len(existsfile) >= 1:
		os.system("rm %s/*_R1.fastq"%outdir)
		os.system("rm %s/*_R2.fastq"%outdir)
	handle = open("%s/%s"%(outdir, outfile), 'w')
	out = csv.writer(handle, delimiter = "\t", lineterminator="\n")
	out.writerow(["SeqId", "Sample_id", "Out5ID", "Out3ID", "Prim5Mis", "Prim5ID", "Prim5pos", "Prim3Mis", "Prim3ID", "Prim3pos", "UMI5", "UMI3", "CDR3"])
	try:
		while True:
			...
			if P3info[0] != 100:
				...
				inferedCDR3 = 'NNNN'
			print([rec1.id] + [sampleid] + [out5, out3] + P5info[:2] +[P5info[3]]+ P3info[:2] + [P3info[3]] + [UMI5, UMI3] + [inferedCDR3])
			out.writerow([rec1.id] + [sampleid] + [out5, out3] + P5info[:2] +[P5info[3]]+ P3info[:2] + [P3info[3]] + [UMI5, UMI3] + [inferedCDR3])
